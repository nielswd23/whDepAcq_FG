...
adj_emb_isl_file = "adjunct_emb_island.txt"

test_nolex, test_lex = format_sequences(s_path + adj_emb_isl_file, lex_phrase_dict)
test_in, test_out, test_mask = data_to_tensor(test_lex, lex_phrase_dict)

# flipping lex phrase dictionary to read model output easier 
out_dict = {value: key for key, value in lex_phrase_dict.items()}
out_dict[0] = "PADDING"
out_dict[1] = "END"

mod_out = check(test_in[0], embedding)
d = []
for i, out in enumerate(mod_out):
    l = []
    for i, prob in enumerate(out):
        l.append((out_dict[i], prob.item()))

    sorted_l = sorted(l, key=lambda x: x[1], reverse=True)
    d.append(sorted_l)

# ## TODO edit this function to return the probabilities of all the continuations at each step 
# def test_score(test_item, item_out, item_mask, emb, phrase_dict):
#     model_output = check(test_item, emb)
